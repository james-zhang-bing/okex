...
type (
	Balance struct {
		...
		// mgnRatio String 美金层面保证金率适用于跨币种保证金模式 和组合保证金模式
	...
	BalanceDetails struct {
		...
		// maxLoan String 币种最大可借适用于跨币种保证金模式和组合保证金模式
	...
	BalanceAndPosition struct {
		// eventType String 事件类型，枚举值：snapshot：首推快照 delivered：交割      exercised：行权 transferred：划转filled：成交      liquidation：强平 claw_back：穿仓补偿 adl：ADL自动减仓      funding_fee：资金费 adjust_margin：调整保证金      set_leverage：设置杠杆 interest_deduction：扣息
	...
	Bill struct {
		...
		// instId String 是 产品ID，如 BTC-USD-190927-5000-C instId和instType不匹配 查询条件中的instId的交易产品当前不是可交易状态，请填写单个instid逐个查询状态详情 instId {0} 报价不可以超过你预设的价格限制
		InstID    string              `json:"instId"`
		// notes String 备注仅适用于资金划转，不是资金划转时，返回 ""
	...
	Config struct {
		...
		// posMode String 持仓方式long_short_mode：双向持仓net_mode：单向持仓仅适用交割/永续
