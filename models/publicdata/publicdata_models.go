...
type (
	Instrument struct {
		...
		// alias String 合约日期别名this_week：本周 next_week：次周quarter：季度next_quarter：次季度 仅适用于交割
	...
	GetDiscountRateAndInterestFreeQuota struct {
		...
		// discountLv String 否 折算率等级1:第一档 2:第二档 3:第三档4:第四档 5:第五档
	...
	LiquidationOrder struct {
		...
		// instType String 产品类型SPOT：币币MARGIN：币币杠杆SWAP：永续合约 FUTURES：交割合约  OPTION：期权 交易产品类型不匹配指数（instType和uly不匹配）
		InstType  okex.InstrumentType       `json:"instType"`
		// totalLoss String 当前underlying下，当期内的总穿仓亏损以USDT为单位，每天16:00（UTC+8）清零
